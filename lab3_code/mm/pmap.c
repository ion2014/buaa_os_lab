...
static void *alloc(u_int n, u_int align, int clear)
{
    ...
    if (freemem == 0) {
        freemem = (u_long)end;//end=0x80400000
    }
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
	...
	if (size%BY2PG) {
		panic("size is not a multipple of BY2PG!\n"); return;
	}
...
void page_init(void)
{
     ...
     * filed to 1) */
	int i;
	int temp = PADDR(freemem)/BY2PG;
	for (i = 0; i < temp; i++)
		pages[i].pp_ref = 1;
	//printf("fpage=%u npage=%u\n", temp, npage);
    /* Step 4: Mark the other memory as free. */
	for (i = temp; i < npage; i++) {
		pages[i].pp_ref = 0;
		LIST_INSERT_HEAD(&page_free_list,&pages[i],pp_link);
		//insert the empty page into the queue
	}
}
//DONE
...
page_alloc(struct Page **pp)
{
     ...
     * Hint: use `bzero`. */
	*pp = ppage_temp;
	//printf("page_alloc : step 3\n");
	bzero(*pp, sizeof(*pp));
	//printf("exit: page_alloc\n");
	return 0;
}
//DONE
...
int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
	...
	if (*pgdir_entryp & PTE_V) {		// presents
		pgtable = (Pte*)KADDR(PTE_ADDR(*pgdir_entryp));
	}	
	else if (create){					//missed
		if (page_alloc(&ppage)) {		//allocate failed
			*ppte = NULL;
			return -E_NO_MEM;
		}
		ppage->pp_ref++;				//add reference to the allocated page
		*pgdir_entryp = page2pa(ppage) | PTE_V | PTE_R;
										//mark the correspoding pde as present
		pgtable = (Pte*)KADDR(PTE_ADDR(*pgdir_entryp));
	}
	else {								//get failed.
		*ppte = NULL;
